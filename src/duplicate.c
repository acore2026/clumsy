...
static Ihandle* dupSetupUI() {
    ...
    registerUiBinding(NAME"-chance", chanceInput, "VALUE", "10.0");
    registerUiBinding(NAME"-count", countInput, "VALUE", STR(COPIES_COUNT));
...
static short dupProcess(PacketNode *head, PacketNode *tail) {
    ...
    while (pac != tail) {
        if (checkDirection(pac->addr.Outbound, dupInbound, dupOutbound)
            ...
            FORWARD_LOG("duplicating w/ chance %.1f%%, cloned additionally %d packets", chance/100.0, copies);
            while (copies--) {
